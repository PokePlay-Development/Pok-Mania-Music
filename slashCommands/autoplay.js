...
module.exports = {
    ...
    run: async (client, interaction) => {
        ...
        if(queue.connection.dispatcher.paused) {
            return interaction.reply({ ephemeral: true, content: `The Song Is Paused.` })
        }
        if(queue.autoplay) {
            queue.autoplay = false;
            interaction.reply({ ephemeral: true, content: `Autoplay Disabled.` })
        } else {
            queue.autoplay = true;
            interaction.reply({ ephemeral: true, content: `Autoplay Enabled.` })
        }
    }
}